...
import { Card, CardMedia, CardContent, Typography, Box, MenuItem, Select } from "@mui/material";

function UpdateCarStatus() {
  const [cars, setCars] = useState([]); // State to hold car data
  const [loading, setLoading] = useState(true); // Loading state
  const [statusOptions] = useState(["Active", "Out of Service", "Maintenance"]); // Status options

  // Fetch cars from the backend
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const response = await fetch("http://localhost/car-rental-website/updateCarStatus.php", {
          method: "GET",
          credentials: "include",
        });
        if (!response.ok) {
            ...
            >
              ...
              <CardContent sx={{ padding: 2 }}>
                ...
                <Box sx={{ marginTop: 2 }}>
                  <Select
                    value={car.status}
                    onChange={(e) => handleStatusUpdate(car.car_id, e.target.value)}
                    fullWidth
                  >
